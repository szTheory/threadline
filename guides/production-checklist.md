...
- [ ] `mix threadline.verify_coverage` fails on any error finding (`:pk_drift`, `:shared_capture_function`, `:duplicate_capture_trigger`, `:capture_trigger_disabled`) for an expected table; a `:legacy_trigger_no_pk_args` warning means "regenerate when convenient", not an immediate problem.
