...
Shipped names and columns come from the migrations generated by `mix threadline.install`: `audit_transactions`, `audit_changes`, `audit_actions`, and the optional `audit_transactions.action_id` link. Threadline stores these relations in the configured `storage_schema` (`public` by default, your host's default schema; a dedicated schema such as `"threadline"` is an opt-in chosen before `mix threadline.install`). Regenerate and review the migration when upgrading across Hex releases.
