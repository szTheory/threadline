...
Shipped names and columns come from the migrations generated by `mix threadline.install`: `audit_transactions`, `audit_changes`, `audit_actions`, and the optional `audit_transactions.action_id` link. Threadline stores these relations in the configured `storage_schema` (`threadline` by default, explicit `public` for the historical footprint). Regenerate and review the migration when upgrading across Hex releases.
