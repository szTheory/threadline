...
| 3 | Framework breadth / more adapters | **Active (v1.19)** | Once adoption is hardened, the next leverage point is reusable host patterns, auth/framework adapters, and a disciplined package-boundary decision. |
| 4 | Production confidence / governance defaults | **Next after v1.19** | Heavier governance knobs (retention admin with capture machinery, runtime policy controls, scale dashboards) inform better once adopters exercise the operator + integration loop in real host apps. |
