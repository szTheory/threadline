...
With v1.26 shipped, phx.gen.auth reach wedge is closed (guide + root CI proof). The library is **~90–93% done** for its stated narrow audit-platform scope. The largest remaining **adopter-facing** gap is **distribution truth** — Hex.pm still lists **0.5.0** while in-repo SSOT is **0.6.0** — plus hidden first-hour config (`ecto_repos` absent from getting-started).
...
| 11 | Distribution & first-hour finish | **Active (v1.27)** | Hex 0.6.0 publish + `ecto_repos` doc + v1.26 audit doc carry-forward |
...
| v1.27 | **active** | Distribution & First-Hour Finish | In-repo done ≠ adopter can install/evaluate; `ecto_repos` hole; Hex lag. | Honest "done for scope"; evaluator path from Hex 0.6.0. | External pilot; compliance expansion; new product surface. |
