...
- [ ] **EXPLORE-01**: `Threadline.timeline/2` and the underlying query surface expose an explicit stable paging contract for large investigation windows, preserving the existing `(captured_at, id)` ordering without pushing adopters toward ad-hoc offset pagination.
