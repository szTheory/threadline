...
- [ ] **WALK-04**: Evidence section — three exercises against shipped subjects only (retention purge run, redaction policy snapshot, trigger coverage snapshot), executed via `mix verify.evidence` / `mix threadline.evidence.show` and the `/audit/evidence` LiveView; expected `Threadline.Evidence` records documented.
