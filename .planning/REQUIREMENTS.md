...
- [x] **XPLO-02**: Integrator can load **all** `%AuditChange{}` rows for a given **`audit_transactions.id`** via a documented **`Threadline.Query`** function and a **`Threadline`** delegator, with **stable ordering** (for incident reconstruction and API responses).
- [x] **XPLO-03**: **`guides/domain-reference.md`** includes a **concise decision guide** (when to use `history/3`, `timeline/2`, export, correlation filters, transaction-scoped listing, diff helpers); at least one **cross-link** from existing **support** or **production-checklist** material; **doc contract test(s)** lock any **new** stable anchors introduced for v1.10.
...
| XPLO-02 | Phase 32 | Complete |
| XPLO-03 | Phase 33 | Complete |
