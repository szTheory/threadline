...
- [x] **WALK-03**: Incident section — three concrete user-uttered scenarios with documented answer procedures using only the shipped operator surface:
...
- [x] **FIX-02**: Every (b) DX papercut finding is fixed in Phase 110 if the fix is ≤ 1 narrow plan in scope; over-budget papercuts are explicitly deferred with a `.planning/v1.24-seeds/SEED-NNN.md` and the finding marked `deferred_to:`.
- [x] **FIX-03**: Every (c) doc gap finding is fixed in `guides/` or example-app README, with `mix verify.doc_contract` updated to lock the literals if doc contracts are touched.
- [x] **DEFER-01**: Every (d) design gap finding lands as `.planning/v1.24-seeds/SEED-NNN.md` with a one-paragraph rationale, the originating finding ID, and a "when this seed should surface" trigger; finding file marked `deferred_to:` and Phase-110 SUMMARY.md lists deferred seeds.
