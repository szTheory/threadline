...
- [ ] **TRUTH-01**: Example `threadline_operator_surface/2` mount passes `evidence_authorize_fn` for admin sessions so `/audit/evidence` is reachable in the sigra-reference lane; support sessions remain denied unless explicitly scoped in docs.
- [ ] **TRUTH-02**: `guides/adoption-pilot-backlog.md` distribution table reflects **threadline 0.5.0** / `~> 0.5` (aligned with `mix.exs` and README doc contract).
- [ ] **TRUTH-03**: Evidence CLI naming aligned repo-wide: canonical `mix threadline.evidence.show`; add `mix verify.evidence` alias **or** doc-contract footnote everywhere PROJECT/v1.22 archives cite `mix verify.evidence` — one chosen approach, locked in tests.
- [ ] **TRUTH-04**: WALK-03-02 operator question prose matches seeded time window (`demo_last_tuesday` / `demo_epoch`) — closes WR-110-001.
- [ ] **TRUTH-05**: `mix verify.doc_contract` and `mix verify.example` green after doc/example changes.
...
| TRUTH-04 | 113 | Pending |
| TRUTH-05 | 113 | Pending |
