...
- [ ] **DOC-02**: A new guide `guides/operator-surface.md` covers mount, the `:actor_fn` / `:authorize_fn` options, the screens, the documented support questions each screen answers, and the `mix threadline.incident` companion task.
- [ ] **DOC-03**: README acquires a top-level "Operator Surface" section pointing to the new guide; `guides/production-checklist.md` gains an operator-surface row; `examples/threadline_phoenix/README.md` documents the wired example end-to-end.
