...
- [x] **Phase 207: Trigger Migration Rerun and Storage-Schema Default Docs** - Make a `mix threadline.gen.triggers` rerun for the same tables produce a migration Ecto accepts (duplicate migration name, audit W1), and correct the stated `storage_schema` default in two guides (W2). (completed 2026-09-24)
...
**Success Criteria** (what must be TRUE):
  ...
  3. `git log origin/main..main` is empty and the latest `main` run concludes `success` in ≤ 20 minutes (target ≤ 12), with every job carrying a `timeout-minutes` bound and a systemically-broken browser suite aborting early rather than accumulating per-test timeouts. (GREEN-06, GREEN-07) **Terminal disposition (2026-08-30): the CI-green clause is accepted as permanently unmet for milestone v1.41 — maintainer-selected option-a at the `198-39` blocking checkpoint. `verify-capture` and `verify-example-browser` (3 named `operator-stress.spec.ts` rows) remain red by construction under D-39, which forbids their only remedy for this milestone. This criterion is NOT restated as met. See `198-39-DECISION.md`.**
...
#### Gap-closure round 14 — canonical-only legacy receipt compatibility (plan 198-62)
...
- The compatibility exception is selected only by the canonical Round-11 paths, exact committed blobs and digests, matching JSON/Markdown decision join, and exact completed execution state. A copy, alias, one-byte mutation, incomplete state, or future schema-v2 record cannot inherit it.
- Every other production or synthetic receipt requires literal argv, `force=false`, ordered timestamps, zero exit, before/after identities, and operation-specific one-object semantics. Fixture stages remain explicit and cannot authorize production.
- The new terminal certificate audits summaries 01-61, names Plan 62 as the sole non-recursive exception, seals every source to one `certified_head`, and records the exact six passing summary/focused/live/branch-protection/terminal/full lanes.
...
- [x] 199-04-PLAN.md — Shared ESM path and safe-write adapter
...
- [x] 199-05-PLAN.md — Critic reader/writer adapter adoption
- [x] 199-06-PLAN.md — Playwright capture adapter adoption
- [x] 199-11-PLAN.md — Hardened cleanup and committed clean-clone proof
...
- [x] 199-13-PLAN.md — Full-app Dialyzer triage and strict ignore ratchet — **HALTED:** sealed first run found 22 warning-origin files against the 14-file maximum; re-plan/re-slice required
- [x] 199-15-PLAN.md — Bounded Dialyzer verifier tracer and critic-tooling remediation
...
- [x] 199-19-PLAN.md — Operator LiveView timer and result-contract remediation
...
- [x] 203-04-PLAN.md — `Design.AliasUsage` sweep, part 2: non-contract tests outside operator_surface (batched by directory)
...
- Rendered HTML is unchanged at every commit: CSS byte lock, rendered_output_contract, copy_contract, component contract render assertions, each LiveView's tests, `mix verify.example`, and the browser lane at exactly the 8 known failures (D-00b).
...
- Credo structural register (from Phase 203; enforced in source by `test/threadline/credo_config_contract_test.exs`, this is a mirror only): Refactor.Nesting 26, Refactor.CyclomaticComplexity 16, ceiling 42, historical max 46; 4 Nesting sites were flattened in place in 203.
...
| 207. Trigger Migration Rerun and Storage-Schema Default Docs | v1.41 | 3/3 | Complete    | 2026-09-24 |
