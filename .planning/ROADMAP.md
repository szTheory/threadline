...
Phase-level specs, success criteria, and plan checklists live in [.planning/milestones/v1.2-ROADMAP.md](milestones/v1.2-ROADMAP.md). Execution artifacts remain under [.planning/phases/](phases/) for phases **09**–**11** until optionally moved with `/gsd-cleanup`.
...
Product requirements and phased roadmap for the next version are **not** defined in-repo yet. Run `/gsd-new-milestone` to open the next planning cycle (creates a fresh `.planning/REQUIREMENTS.md`).
