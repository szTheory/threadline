...
- **D-09b:** Watch the secondary inconsistency (not the bug itself, fix opportunistically): the inner `tl-transaction tl-short-content` wrapper (`max-width:72ch; margin-inline:auto`, `style.ex:2887-2890`) only wraps the page header, not `#changes-list`/`tl-viewport` — so the changes list is full-width even after the outer fix. Planner to decide whether to align it for visual consistency.
...
- **D-11:** **Connection-class anchor confirmed: `.threadline-ui.phx-loading` / `.threadline-ui.phx-error`.** The LiveView render-root *is* `.threadline-ui` in all 11 views, so phoenix_live_view applies its client classes directly there — no parent indirection. The banner CSS (hidden→`flex`, `[data-tl-mutating]`→`opacity:.55;pointer-events:none`) at `style.ex:3405-3424` is already correct (177). **Never** `<body>`, **never** the legacy `.phx-disconnected`.
...
- `.planning/phases/177-component-groups-meta-components/177-CONTEXT.md` — **most load-bearing.** `stack`/`cluster`/`data_panel`/`toolbar`/`detail_header`/`reconnect_banner`, the offline group keyed off `.threadline-ui.phx-loading/.phx-error`, group motion + reduced-motion blanket, cross-child state coordination. The pages are assembled from these.
