...
**In scope:** automated accessibility checks against rendered pages and opened overlays/disclosures; keyboard/focus and browser accessibility-tree evidence with explicit assistive-technology proof limits; focus entry/restore/non-obscured focus proof; WAI-ARIA APG review for dialog, tabs, menu/dropdown, combobox, disclosure, tooltip, table/grid, alert, and nav patterns; touch target and non-color-only checks; motion audit against compositor-safe transform/opacity, origin awareness, no `scale(0)`, press feedback, reduced motion, and token boundaries; expanded Playwright/stress screenshot guardrails; brand-token parity, style/component/copy/stress contracts; adversarial regression review and milestone closeout evidence.
...
- **D-01:** Use a **layered proof model** instead of trying to inspect every theoretical state. Tier A is automated and broad: ExUnit/style/component/stress contracts plus browser checks on representative rendered states. Tier B is real-engine browser coverage: opened overlays, disclosures, keyboard paths, reduced motion, light/dark/system, and stress-route screenshot/geometry guards. Tier C is bounded accessibility-tree evidence: Playwright ARIA snapshots for representative flows, explicitly not real screen-reader UAT.
- **D-02:** The final Phase 180 report must state exactly what each tier proves. Do not let green automation imply that every page/state/theme/viewport or assistive-technology pairing was manually reviewed. Use the Phase 178 honesty pattern: full structural coverage where deterministic, representative browser coverage where high-signal, and explicit proof limits where real assistive-technology judgment is not run.
...
- **D-05:** Keyboard and accessibility-tree evidence is required but bounded. Record deterministic checklists as artifacts: keyboard traversal, focus visibility, focus entry/restore, Escape/click-outside behavior, and browser accessibility-tree snapshots for landmark/headings/labels/status/alert/dialog states. Do not claim subjective full human UAT or real screen-reader coverage unless that UAT is actually run.
...
- Use the Phase 178 wording pattern, adapted to the executed evidence: "Tier A proves the full structural matrix; Tier B proves representative real-engine cells; Tier C proves sampled browser accessibility-tree structure, not real assistive-technology judgment." This keeps closeout honest.
