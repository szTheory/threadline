...
### Claude's Discretion
- Exact JSON/field spelling in `golden-set.json`, `critic_trust`, the refute manifest, and the `.planning/critic-scores/` files (beyond the locked shapes above); exact `e2e/critic/` file names; test-name wording — planner/executor choose, consistent with existing conventions.
...
- The precise `band` enum thresholds mapping 0–100 → `fail|weak|ok|strong|exemplary` (must align with the 194 anchored-band boundaries).
