...
|----------|-------------|------------|-------------------|
| Golden-set labeling, refute proof, and trust measurement (195-07 Task 3) | CRITIC-01 | Requires `ANTHROPIC_API_KEY` + human aesthetic judgment; runs outside CI; rescoring is never auto-green | 1. Load the key: `set -a; source .env; set +a` (never commit .env / the key). 2. Seed + blind-label: `cd examples/threadline_phoenix/e2e && npm run critic:label -- --bootstrap`, complete `--round r1`, commit `rounds/r1.json`, then `--round r2` (reshuffled), then `--reconcile`; target ≥20 lens-judgments/critic-bearing lens (`--status` shows the bar; lenses under 20 stay `provisional`). 3. Prove the critic (refute battery, ~$1–3): `npm run critic:validate` (dry-run first). 4. Score + measure: `npm run critic:score -- --golden` (scores labeled cells), then `mix critic.measure` writes per-lens `critic_trust` (a lens is `validated:true` ONLY if α≥0.67 AND n≥20 AND raw≥80% at current rubric+model). 5. Regenerate: CRITIQUE.md auto-regens after `critic:score`; standalone `node --import tsx critic/run.ts report` → confirm `.planning/CRITIQUE.md` is fresh. 6. Confirm CI stays honest: `mix ci.all` green. Commit golden set, rounds, critic-scores, CRITIQUE.md, and the `critic_trust` block as one reviewed commit. |
