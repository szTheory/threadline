...
| **Full suite command** | `mix ci.all` plus `mix verify.example_browser_light` for Phase 183 light/system theme proof if the full suite does not include that lane |
...
- **Before `/gsd:verify-work`:** Run `mix ci.all`, `mix verify.example_browser`, and `mix verify.example_browser_light`, or document any environment failure with the exact failing command and artifact.
...
| 183-GATE | TBD | final | SHELL-01, SHELL-02, SHELL-03 | T-183-01..03 | Phase closes only when shell/Home source contracts and browser perception lanes prove navigation, hierarchy, theme, focus, feature-gate, and overflow behavior. | full suite + evidence | `mix ci.all`, `mix verify.example_browser`, and `mix verify.example_browser_light` | commands exist | pending |
...
- [ ] `mix ci.all`, `mix verify.example_browser`, and `mix verify.example_browser_light` are green or residual failures are classified in `183-VERIFICATION.md` with evidence they are inherited and unrelated.
