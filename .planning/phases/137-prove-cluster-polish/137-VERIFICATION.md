...
overrides_applied: 0
human_verification:
  - test: "Inspect `/audit/exports` at 375px with ready, preparing, failed, expired, and file-missing jobs."
    ...
    why_human: "Viewport hierarchy and text fit require visual inspection."
  - test: "Inspect `/audit/policy/retention` at 375px with completed, queued, and failed runs."
    ...
    why_human: "Mobile visual order and chip sizing cannot be fully proven by static grep/tests."
  - test: "Inspect `/audit/evidence` and `/audit/policy/redaction` at 375px in dense states."
    ...
    why_human: "Dense mobile readability and visual hierarchy require human UAT."
---
...
**Test:** Inspect `/audit/exports` at 375px with ready, preparing, failed, expired, and file-missing jobs.
**Expected:** Actor refs/query params stay secondary and truncated, readiness group headings are visible, and only ready downloads read as the primary action.
**Why human:** Viewport hierarchy and text fit require visual inspection.

### 2. Retention Dense Mobile State

**Test:** Inspect `/audit/policy/retention` at 375px with completed, queued, and failed runs.
**Expected:** Summary/latest-completed/failure context visually precedes `Run retention prune`, the failure-count anchor target is discoverable, and status chips remain max-content.
**Why human:** Mobile visual order and chip sizing cannot be fully proven by static grep/tests.

### 3. Evidence And Redaction Dense Mobile State

**Test:** Inspect `/audit/evidence` and `/audit/policy/redaction` at 375px in dense states.
**Expected:** Evidence verdict/subject lead before subject refs, `Open proof history` remains the first card action, and Redaction details/status chips stay readable without orphaned full-width badges.
**Why human:** Dense mobile readability and visual hierarchy require human UAT.

### Gaps Summary

No automated gaps found. Status is `human_needed` solely because the phase plans include deferred 375px dense-state visual checks, and visual hierarchy/readability must be confirmed by a human before the phase is marked fully passed.
