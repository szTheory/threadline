...
| M-02 | `.threadline-ui .tl-shell-nav__item` | Header navigation | Hover and active-route changes | P1/P4: keep current operator context and destination changes readable during triage | Restrained color/background/border/shadow transition communicates navigation state without moving layout | `--tl-transition-fast` | color, background-color, border-color, box-shadow | Frequent, user-triggered | `prefers-reduced-motion` collapses transition duration through `.threadline-ui *` | `style.ex` `.threadline-ui .tl-shell-nav__item` | keep |
