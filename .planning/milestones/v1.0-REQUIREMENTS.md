...
**Outcome:** All 51 v1 requirements marked complete for the shipped library scope. Registry publish (`mix hex.publish`) is tracked separately in `PROJECT.md` as a maintainer gate.
...
- [x] **QUERY-01**: `Threadline.history(schema_module, id)` returns a list of `AuditChange` records for the given record, ordered by `captured_at` descending
- [x] **QUERY-02**: `Threadline.actor_history(actor_ref)` returns a list of `AuditTransaction` records associated with the given actor
- [x] **QUERY-03**: `Threadline.timeline/1` accepts filter options (`:table`, `:actor_ref`, `:from`, `:to`) and returns a combined list of `AuditChange` records
- [x] **QUERY-04**: All query functions accept an `Ecto.Repo` option so callers can target a specific repo in multi-repo setups
- [x] **QUERY-05**: Query results are plain Ecto structs, not opaque library types — callers can pipe results through standard `Ecto.Query` composition

### Health & Observability

- [x] **HLTH-01**: `Threadline.Health.trigger_coverage/0` returns a list of audited tables and whether each has a valid trigger installed in the current PostgreSQL schema
- [x] **HLTH-02**: A table with no trigger installed is reported as `{:uncovered, table_name}` in the coverage result
- [x] **HLTH-03**: `:telemetry` event `[:threadline, :transaction, :committed]` is emitted after each `AuditTransaction` is committed, with `%{table_count: n}` as measurements
- [x] **HLTH-04**: `:telemetry` event `[:threadline, :action, :recorded]` is emitted after each `AuditAction` is persisted, with `%{status: :ok | :error}` as measurements
- [x] **HLTH-05**: `:telemetry` event `[:threadline, :health, :checked]` is emitted when `trigger_coverage/0` is called, with `%{covered: n, uncovered: m}` as measurements
...
- [x] **DOC-01**: README contains a working installation example (add dep, run install task, configure Plug) that a developer can follow in under 15 minutes
- [x] **DOC-02**: README links to a domain reference document that defines: AuditTransaction, AuditChange, AuditAction, AuditContext, ActorRef, and Correlation
- [x] **DOC-03**: README documents the PgBouncer transaction-mode constraint and the safe context propagation pattern Threadline uses
- [x] **DOC-04**: CONTRIBUTING.md skeleton exists and describes how to run the test suite and submit a PR
- [x] **DOC-05**: All public API modules have `@moduledoc` and all public functions have `@doc` strings
