# Requirements Archive: v1.36 Operator Surface Light Mode
...
- [x] **COMP-01**: The ~9 dark-effect component families (glass chrome ×4, drawer scrim + shadow, focus glow, home-card signature effects, shell-nav inset) render correctly on light with explicitly designed treatments. *(Complete: built + user-approved live both modes; `style.ex` source committed at milestone close (commit `b9f8fc1`), resolving `v1.36-MILESTONE-AUDIT.md` F1.)*
- [x] **COMP-02**: Data-viz-adjacent surfaces (coverage, timeline, diff views) pass an explicit light-mode design review — no dark-tuned content shipped unreviewed into light (the Grafana lesson). *(Complete: coverage row hover-polarity fix + data-viz light review done and user-approved live; `style.ex` source committed at milestone close (commit `b9f8fc1`), resolving `v1.36-MILESTONE-AUDIT.md` F1.)*
...
| COMP-01 | Phase 167 | Complete |
| COMP-02 | Phase 167 | Complete |
