# Milestone archive: v1.3 — Production adoption (redaction, retention, export)
...
Production-adoption milestone: **capture-time redaction** so secrets never land in audit JSON; **retention + batched purge** so operators can bound growth safely; **CSV/JSON export** aligned with `Threadline.Query.timeline/2` filters for support and ops workflows.
...
- ◆ **v1.3 — Production adoption (redaction, retention, export)** — Phases 12–14 (pre-close snapshot; requirements archived as `v1.3-REQUIREMENTS.md`)
...
### v1.3 — Production adoption (execution complete)

Continues phase numbering after v1.2 (last shipped phase **11**). Full requirement text (archived): `.planning/milestones/v1.3-REQUIREMENTS.md`.
