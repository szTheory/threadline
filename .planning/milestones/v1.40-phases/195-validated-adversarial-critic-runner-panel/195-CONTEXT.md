...
### D-12: Synthetic twin oracle — labeling-free validation that may drive the forward-only ratchet (REVERSES part of D-01/D-03)
- **Context / why:** the human golden-set labeling loop (≥20 items/lens × 2 blind rounds) proved too costly for the solo maintainer to sustain. Since the α/trust engine (`measure.ex`) is **label-source-agnostic** — it only needs `golden-set.json`-shaped items carrying an `r1.verdict` — the oracle can instead be **constructed**: a graded severity ladder of twins (lens × scenario × 4 rungs r4/r3/r2/r1) whose verdicts (good/borderline/bad/broken) are **definitional, not observed**. This reaches n≥20/lens with **zero human labeling**.
- **This explicitly reverses** the synthetic-exclusion clauses of **D-01** ("synthetic degradations are reserved for the refute battery, NOT the golden set") and **D-03** ("never fold refute fixtures into the agreement metric — no teaching-to-the-test"). The reversal is deliberate and recorded here; it is not a silent swap.
- **The weaker claim (state it plainly):** synthetic validation proves the critic **tracks known-severity injected flaws monotonically on held-out interpolation rungs** — it does **NOT** prove the critic matches the maintainer's taste on ambiguous real UI. It is a strictly narrower, *calibration* claim, not a *taste* claim.
- **Anti-circularity:** rubrics keep anchoring their few-shot pole exemplars on the **binary extremes** (`refute.<lens>.<twin>.polished/flawed`); the graded ladder cells get **distinct held-out cell-ids** (`refute.<lens>.graded.<scenario>.<rung>`) the rubric never showed the critic — so α measures *interpolation*, not memorization. The `critic_trust_test.exs` pole-disjointness guard is extended to enforce this against the synthetic set too. Residual risks (shared-axis tautology at band boundaries; pseudo-replication inflating n) are mitigated by honesty (distinct scenarios, effective-N reporting), not hidden.
- **Trust bar for the ratchet (maintainer decision):** a synthetic `validated:true` **is accepted as sufficient to drive the Phase-196 forward-only gate**, which only ever asks "did this change get *worse*?" (a delta) — the exact property calibration proves. It is NOT a claim of human-equivalent trust for absolute-quality judgments; PR review remains the backstop for novel/subjective calls.
- **Honest provenance:** the oracle is recorded in a sibling `critic_trust_provenance` ledger block (`oracle: "synthetic" | "human"`, `generated_from: "graded-twin-ladder"`) and in `synthetic-set.json` (`golden_source: "synthetic"`). Guards forbid an unlabeled promotion (validated:true ⇒ non-null oracle). Human labeling is retained as an optional stronger-claim upgrade path, never required.
- **Mechanics:** `mix critic.synth` generates `.planning/golden/synthetic-set.json` from the graded catalog; `npm run capture:graded` shoots the rung cells; `npm run critic:score -- --synthetic` scores exactly the (cell, lens) pairs; `mix critic.measure --source synthetic` computes α + writes provenance. `golden-set.json` stays pristine/human-only.
- **Gate revision (α is the gate; raw demoted):** empirically the critic RANKS severity correctly (rhythm α≈0.78) but does NOT exact-match 4 buckets (rhythm raw≈0.43) — it compresses a scale it orders well (e.g. even/good rhythm reads `ok`, not `strong`), which is the nature of an LLM gestalt critic. So the trust bar is revised: `validated = α≥0.67 ∧ n≥20 ∧ model-pin ∧ fresh-rubric`. **`raw_agreement` is still computed and emitted but no longer gates** (demoted from the original `raw≥0.80`). Rationale: α is chance-corrected, ordinal, and distance-aware — the correct metric for a critic that ranks severity and drives a *forward-only* ("did it get worse?") ratchet; exact-bucket calibration is not required for that job. This applies to both the human and synthetic oracles.
