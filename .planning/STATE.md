...
**Cut Hex 0.4.0 after Phase 64 plan-phase, before execution starts.** v1.17 (Operator Surface Foundation) shipped 2026-05-06; CHANGELOG.md `[Unreleased]` is loaded with v1.16/v1.17 work (operator surface, optional Phoenix deps, `:correlation_id` filter, export `action` object, CSV `include_action_metadata`, examples/threadline_phoenix composition demo, telemetry table). 52 commits since the last v1.16 milestone tag. v1.18 has not yet introduced any new code surface (only Phase 64 PLAN.md docs), so 0.4.0 captures v1.17 cleanly without scope bleed. Resume: stage CHANGELOG `[Unreleased]` → `[0.4.0] - 2026-05-07`, bump `@version "0.3.0"` → `"0.4.0"` in mix.exs, run release pipeline.
...
- 2026-05-07: Pending release decision — cut Hex 0.4.0 between Phase 64 plan-phase and execute-phase. v1.17 just shipped (2026-05-06); 52 commits since the v1.16 milestone tag; CHANGELOG `[Unreleased]` is loaded with operator surface + correlation_id filter + export action object + CSV include_action_metadata + composition demo + telemetry table — coherent semver-minor. v1.18 hasn't shipped any new code surface yet, so cutting before Phase 64 execution avoids scope bleed; v1.18 closes with 0.5.0.
