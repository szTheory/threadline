defmodule Threadline.GettingStartedSaasDocContractTest do
  ...
  test "quickstart guide locks the adopter walkthrough" do
    ...
    assert String.contains?(doc, "{:threadline, \"~> 0.5\"}")
    ...
    assert String.contains?(doc, "mix threadline.export --dry-run --table posts")
    ...
    assert String.contains?(doc, "support-read-only")
