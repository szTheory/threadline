...
defmodule IncidentReplay do
  def run do
    ...
    if not execute? do
      IO.puts(
        format_json(:info, "Dry run enabled. Pass --execute to perform mutations.",
          incident: incident
        )
      )
  ...
  defp set_context_guc(repo, context_map) do
    ...
    Ecto.Adapters.SQL.query!(repo, "SELECT set_config('threadline.actor_ref', $1::text, false)", [
      json
    ])
  end
  ...
  defp scenario_who_changed_row do
    post =
      case Repo.all(Post) |> List.first() do
        nil ->
          %Post{}
  ...
  defp scenario_service_account do
    ...
    changes =
      Ecto.Adapters.SQL.query!(
        Repo,
        "SELECT * FROM #{audit_transactions_table()} WHERE actor_ref->>'id' = $1",
        [service_account_id]
      )

    IO.puts(
      format_json(:success, "Scenario completed",
        incident: "service-account-today",
        changes_count: changes.num_rows
      )
    )
  end
  ...
  defp scenario_oban_job do
    ...
    |> Ecto.Changeset.change(%{
      title: "Oban Post",
      slug: "oban-#{System.unique_integer([:positive])}"
    })
    |> Repo.insert!()
    ...
    changes =
      Ecto.Adapters.SQL.query!(
        Repo,
        "SELECT * FROM #{audit_transactions_table()} WHERE actor_ref->>'id' = 'oban-worker'"
      )

    IO.puts(
      format_json(:success, "Scenario completed",
        incident: "oban-job-mutation",
        changes_count: changes.num_rows
      )
    )
  end

  defp audit_transactions_table do
    Threadline.StorageSchema.table("audit_transactions")
  end
